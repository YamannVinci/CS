...
RH = st.sidebar.slider("Rebound Number", 14, 55.50, step=0.1)
UPV = st.sidebar.slider("Ultrasonic Pulse Velocity", 1.82, 5.22, step=0.1)

def csMPa():
    my_dict = {"RH" : RH,
              "UPV" : UPV}
    df_sample = pd.DataFrame.from_dict(my_dict, orient='index').T
    return df_sample
